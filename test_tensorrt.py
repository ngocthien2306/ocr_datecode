...
class SuperPointMatcherTensorRT:
    def __init__(self, json_path: str, pipeline_path: str, scale: float = 1.0, 
                 use_fp16: bool = True, verbose: bool = False):
        ...
        self.pipeline_sess = ort.InferenceSession(
            pipeline_path, 
            sess_options=sess_options,
            providers=providers
        )
        
        actual_provider = self.pipeline_sess.get_providers()[0]
        if self.verbose:
            print(f"⏱️  Load pipeline: {(time.time()-t0)*1000:.1f}ms")
        
        print(f"✅ Initialized SuperPointMatcherTensorRT ({(time.time()-t_start)*1000:.1f}ms)")
        ...
        if actual_provider == 'TensorrtExecutionProvider':
            print(f"   ⚠️  First run will build TensorRT engine (~1-2 min), then will be cached")
    
    def _resize_to_1024(self, img):
        h, w = img.shape
        if h != 1024 or w != 1024:
            resized = cv2.resize(img, (1024, 1024))
            return resized, (w / 1024, h / 1024)
        return img, (1.0, 1.0)
    ...
    def match(self, target_path: str, score_threshold: float = 0.3, 
        ...
        t0 = time.time()
        template_resized, template_scale = self._resize_to_1024(self.template_gray)
        target_resized, target_scale = self._resize_to_1024(target_gray)
        timings['resize_to_1024'] = (time.time() - t0) * 1000
    ...
    def _print_timings(self, timings: Dict):
        ...
        print(f"   Resize to 1024x:       {timings['resize_to_1024']:7.1f}ms")
...
if __name__ == '__main__':
    matcher = SuperPointMatcherTensorRT(
    ...
    result = matcher.match('images/1.jpg')
    
    if result['success']:
        print(f"\n✅ Success! Confidence: {result['confidence']:.1%}")
        matcher.visualize(result, save_path='result.png', show=False)
