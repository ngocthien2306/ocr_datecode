...
_CAPTURED_LOGGERS = ("app.services.ocr_training",)
...
class TrainingLogBuffer:
    ...
    def drop(self, project_id: str, model_id: str) -> None:
        """Clear in-memory state and delete the on-disk log -- call this from
        model deletion so a removed model doesn't leave its log file behind
        (delete_model already removes the checkpoint/export files for the same
        reason).

        The path is recomputed from (project_id, model_id) rather than read out
        of self._log_paths: that map is only populated by register(), so a model
        trained before the last service restart had no entry and its log file
        survived the delete. anomaly_service's copy of this class still has that
        bug.
        """
        with self._lock:
        ...
        _log_file_path(project_id, model_id).unlink(missing_ok=True)
