...
from app.agent.prompts.orchestrator_prompts import ORCHESTRATOR_SYSTEM_PROMPT
...
class OrchestratorAgent(BaseAgent):
    ...
    Routing targets:
    - service_management: Service status, start/stop, logs
    - historical_analytics: Statistics, trends, history
    - (future agents...)
    """

    def __init__(self, agent_id: str, model_name: str = "gpt-4o-mini", temperature: float = 0.2, **kwargs):
...
""")
        ...
        def parse_routing_decision(state: AgentState):
            ...
            try:
                ...
                # Valid routing
                if agent_id in ["service_management", "historical_analytics"]:
