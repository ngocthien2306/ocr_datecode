...
class RecipeBase(BaseModel):
    ...
    cv_method: Optional[str] = Field(default="v4", description="CV pipeline variant when classifier_backend='embedding': 'legacy' (original blur_tm+iou+px) | 'v4' (scale-invariant directional) | 'shape_v7' (gradient orientation)")
    ...
    # Persist PASS frames to disk (default ON). PASS images are kept as a ring
    # buffer of the 200 most-recent per camera per recipe (older ones pruned).
