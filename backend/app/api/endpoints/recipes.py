...
# Pure-cv2 detectors loaded off disk WITHOUT importing the camera_management
# package (that chain pulls in pycuda and makes exiting threads abort the
# process). See app/services/cv_detectors.py.
from app.services.cv_detectors import load_color_verifier, load_image_proc_detector
...
async def detect_bottle_preview(body: dict):
    ...
    # Load color_verifier.py straight off disk. Importing the real
    # camera_management package would drag pycuda into the backend and make any
    # exiting thread abort the process - see app/services/cv_detectors.py.
    try:
        ColorVerificationService = load_color_verifier().ColorVerificationService
    except Exception as e:
        raise HTTPException(503, f"color_verifier load failed: {e}")
...
async def detect_walls_preview(body: dict):
    ...
    try:
        _ipd = load_image_proc_detector()
        EdgeParams = _ipd.EdgeParams
        detect_template_walls = _ipd.detect_template_walls
        detect_product_box = _ipd.detect_product_box
        detect_product_box_from_regions = _ipd.detect_product_box_from_regions
    except Exception as e:
        raise HTTPException(503, f"image_proc_detector load failed: {e}")
...
async def detect_cap_edges_preview(body: dict):
    ...
    try:
        _ipd = load_image_proc_detector()
        EdgeParams = _ipd.EdgeParams
        detect_cap_axis_from_regions = _ipd.detect_cap_axis_from_regions
    except Exception as e:
        raise HTTPException(503, f"image_proc_detector load failed: {e}")
