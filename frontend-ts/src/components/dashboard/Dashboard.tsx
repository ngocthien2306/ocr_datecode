...
import OCRTrainingPage from '../ocr-training/OCRTrainingPage';
...
type Section = 'dashboard' | 'users' | 'receipts' | 'realtime' | 'cameras' | 'historical' | 'settings' | 'logs' | 'documentation' | 'system' | 'ml-training' | 'anomaly-training' | 'ocr-training';
...
export default function Dashboard({ onLogout }: DashboardProps) {
  ...
  // OCR Training Studio. Like anomaly-training, no entry-confirm dialog: OCR
  // fine-tuning was measured at ~2.6 GB against ai_services' 3.5 GB on a 16 GB
  // card, so it coexists with live inference. Concurrent runs are what cannot
  // share the GPU, and ocr_service serialises those on its own lock.
  if (currentSection === 'ocr-training') {
    return (
      <>
        <OCRTrainingPage onClose={() => setCurrentSection('dashboard')} />
        <AgentChatWidget />
      </>
    );
  }
      ...
      <div className={`main-content-wrapper sidebar-${sidebarPosition} ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
        ...
        <aside className="sidebar">
          ...
          <nav className="action-menu">
            ...
            <a
              href="#ocr-training"
              className={`nav-item ${(currentSection as Section) === 'ocr-training' ? 'active' : ''}`}
              onClick={(e) => { e.preventDefault(); handleSectionChange('ocr-training'); }}
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                <path d="M4 7V5a1 1 0 011-1h14a1 1 0 011 1v2" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                <path d="M9 20h6M12 4v16" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
              </svg>
              OCR Training
            </a>
            {canAccessPage('logs') && (
