...
from agent_app.core.backend_client import get_camera_ws_connected
...
def check_service_status(service_name: str = "camera_management") -> Dict[str, Any]:
    ...
    if service_name == "camera_management":
        # WebSocket state sống trong process backend → phải hỏi qua HTTP.
        # None = không gọi được backend (khác hẳn với "chưa kết nối").
        ws_connected = get_camera_ws_connected()
        ...
        elif ws_connected is None:
            status = "unknown"
            message = (
                f"Service đang chạy (PID {pid}) nhưng KHÔNG kiểm tra được WebSocket "
                f"vì không kết nối được backend tại {settings.BACKEND_URL} ⚠️"
            )
        elif ws_connected:
            status = "healthy"
...
def _tail(path: Path, n: int, max_bytes: int = 512_000) -> tuple[list[str], int]:
    """
    Đọc n dòng cuối bằng cách seek từ cuối file.

    Bản cũ dùng f.readlines() — nạp TOÀN BỘ file vào RAM chỉ để lấy vài chục
    dòng cuối. Đo trên log thật (654 MB): 4,69 giây và 1,1 GB RAM đỉnh. Trên
    Jetson, cùng lúc camera service đang giữ ~2 GB, đó là rủi ro OOM thật —
    và log chỉ ngày một to thêm.

    Returns:
        (danh sách dòng, kích thước file tính bằng byte)
    """
    size = path.stat().st_size
    block = min(max_bytes, size)
    ...
    # Nếu không đọc từ đầu file thì dòng đầu tiên gần như chắc chắn bị cắt dở.
    if block < size and result:
        result = result[1:]

    return [ln.rstrip() for ln in result[-n:]], size
...
def get_service_logs(service_name: str, lines: int = 50) -> Dict[str, Any]:
    ...
    if service_name == "camera_management":
        ...
        try:
            last_lines, size = _tail(log_file, lines)

            return {
                "logs": last_lines,
                "returned_lines": len(last_lines),
                "log_file_mb": round(size / 1e6, 1),
                "log_file": str(log_file),
                "message": f"📝 Đọc {len(last_lines)} dòng cuối (file {size / 1e6:.0f} MB)"
            }
