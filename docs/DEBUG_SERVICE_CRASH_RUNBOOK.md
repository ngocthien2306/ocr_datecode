...
|---|---|---|
| **Backend** (uvicorn) | `ocr-backend.service` (systemd, `Restart=always`) — hoặc `nohup` inline nếu :8000 chưa listen | app: `logs/backend/YYYY-MM-DD.log`<br>stdout: `logs/start_services/backend.log` |
| **AI camera service** | `nohup python camera_management_service.py` từ `start_services.sh:187` — **KHÔNG phải systemd** | app: `logs/camera_management/YYYY-MM-DD.log`<br>stdout: `logs/start_services/ai_camera.log`<br>khi BE respawn: `ai_services/logs/camera_management.log` |
