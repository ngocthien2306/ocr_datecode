...
- See [ocr-training.md](ocr-training.md) — new `ocr_service` (fine-tune SVTRv2+SMTR on recipe text/datecode crops → ONNX → TensorRT) + recipe binding via `ocr_project_id`/`ocr_model_id`; Phase 0 verified on real GPU; base-checkpoint selection, `use_space_char` vocab expansion, and the `main_indicator: acc` checkpoint-selection trap
...
9b. **EXCEPTION to #9 — the OCR backend IS hot-swappable per recipe.** `camera_manager.py:299` calls `inference_handler.set_ocr_model_type(recipe_data['ocr_model_type'])` and, if it changed, submits `_reinit_ocr_backend()` to the worker thread. So #9 applies to the char classifier + anomaly model, NOT to OCR recognition models. See [ocr-training.md](ocr-training.md).
